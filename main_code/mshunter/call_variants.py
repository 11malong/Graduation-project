...
# from mshunter.global_dict import * #原来的
...
def call_variants(df_microsatellites):
    ...
    for contig, contig_len in contigs_info.items():
        ...
        win_num = 0
        window_sub = []
        len_bed=len(df_microsatellites)

        if args["ms_num"] < args["batch"]:
            args["batch"] = args["ms_num"] - 1
        ...
        for ms_id, info in this_contig_microsatellite.iterrows():
            ...
            if ms_num % (args["batch"]) == 0 or ms_num>=len_bed:
                window_ms.append(window_sub)
                window_sub = []
                win_num += 1

                if win_num < args["threads"]:
                    args["threads"] = win_num
